...
def NOTIFIED_UPDATE_GROUP(op):
    try:
                sendMessage(op.param1, client.getContact(op.param2).displayName + ", Do Not Playing With The QR :3\nill kick You :V")
                client.kickoutFromGroup(op.param1,[op.param2])
    except Exception as e:
        print e
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                if msg.text in ["Key","Help","help"]:
                    sendMessage(msg.to,"✞ COMMAND GRUP✞\n\n✞ Me\n✞ Mid @\n✞ Mysp\n✞ Mygrup\n✞ Mygid\n✞ Myginfo\n✞ My gurl\n✞ My url\n✞ Link on\n✞ Link off\n✞ Summon\n✞ Cctv 「Set Point Read」\n✞ Show 「View Reader」\n\n✞ COMMAND SPAM/Broadcast ✞\n\n✞ 「Bc:ct/grup Text」\n✞ 「Spam on/off Amount Teks」\n✞ 「Spam @>\n\n✞ COMMAND Profile ✞\n\n ✞ 「Mybio/name 「Text」\n✞ 「Mysteal @」\n✞ 「Mycopy @」\n✞ Mybackup\n✞ Block @\n✞ Blocklist\n\n  ✞ COMMAND Kicker ✞\n\n✞ 「Savage」\n✞ 「Kick @」\n✞ 「Uni」\n\n ฿Ɏ ĐłVł₦Ɇ ₮Ɇ₳₥ ฿Ø₮")                    
                if msg.text in ["Mysp","Speed","speed"]:
                    start = time.time()
                    sendMessage(msg.to, text="Progress...", 
contentMetadata=None, contentType=None)
                   ...
                   if targets == []:
                       ...
                       for target in targets:
                           try:
                               ...
                               pass
                elif "Savage" in msg.text:
                       nk0 = msg.text.replace("Savage","")
                       nk1 = nk0.lstrip()
                       nk2 = nk1.replace("all","")
                       nk3 = nk2.rstrip()
                       _name = nk3
